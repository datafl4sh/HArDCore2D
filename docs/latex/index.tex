...
\item \href{#hybridcore}{\tt The Hybrid\+Core structure} -- How to use the \hyperlink{classHArDCore2D_1_1HybridCore}{Hybrid\+Core2d} structure to compute integrals on the cells and faces of the mesh, and how to use the polynomial basis functions
\end{DoxyItemize}
...
\item The follwing Boost C++ libraries (\href{http://www.boost.org/}{\tt http\+://www.\+boost.\+org/})
\begin{DoxyItemize}
...
Make sure that you have the development version of boost installed. On Linux, install {\ttfamily libboost-\/dev}, {\ttfamily libboost-\/filesystem-\/dev} and {\ttfamily libboost-\/program-\/options-\/dev} from your package manager.
...
After it is loaded, the mesh is represented by classes describing a vertex, an edge and a mesh\+: \hyperlink{classHArDCore2D_1_1Vertex2D}{Vertex2D}, \hyperlink{classHArDCore2D_1_1Edge2D}{Edge2D}, and \hyperlink{classHArDCore2D_1_1Cell2D}{Cell2D}. Each of these classes contains methods to access useful information for the corresponding element, including other geometrical quantities it is related to. The mesh itself is represented by an element of the \hyperlink{classHArDCore2D_1_1Mesh2D}{Mesh2D} class with methods to access all the vertices, edges and cells (or a particular vertex, edge or cell). In this class, each cell has a unique identifier, numbered from 0.

For example, if {\ttfamily mesh\+\_\+ptr} is a pointer to a Mesh2D class, 
\begin{DoxyCode}
Vertex2D* vertex = mesh\_ptr->\hyperlink{classHArDCore2D_1_1Mesh2D_ac29a7c5c0c36b3aa519520fbb9cf20de}{vertex}(5);

Eigen::Vector2d vert\_coord = vertex->\hyperlink{group__Mesh2D_gaa4022dfbf307b635bd6e57a942ab336e}{coords}()
\end{DoxyCode}
...
\textcolor{keywordflow}{for} (\textcolor{keywordtype}{size\_t} iC = 0; iC < mesh\_ptr->\hyperlink{group__Mesh2D_ga49d8009eeb74fd880ef9bb61c37fa672}{n\_cells}() iC++) \{

    \textcolor{comment}{// We grab the edges of the iC-th cell}
    std::vector<Edge2D *> edges = mesh\_ptr->\hyperlink{classHArDCore2D_1_1Mesh2D_a1d8c40ea85f0abe3ae5aea23e4a95583}{cell}(iC)->\hyperlink{classHArDCore2D_1_1Cell2D_ad3c7842a181976770dff4d8ce71c525d}{get\_edges}();
...
A mesh file must be read using an instance of the {\ttfamily Mesh2\+D\+Reader\+Typ2} class, and then built using {\ttfamily Mesh2\+D\+Builder}. A working example is given below (assuming the executable will be in {\ttfamily build/\+Schemes} for example).


\begin{DoxyCode}
\textcolor{preprocessor}{#include "mesh2d.hpp"}
\textcolor{preprocessor}{#include "import\_mesh2d.hpp"}
\textcolor{preprocessor}{#include "mesh2d\_builder.hpp"}
...
\textcolor{keywordtype}{int} main() \{
    ...
    \hyperlink{classHArDCore2D_1_1Mesh2DReaderTyp2}{Mesh2DReaderTyp2} mesh(mesh\_file);
    ...
    \hyperlink{classHArDCore2D_1_1Mesh2DBuilder}{Mesh2DBuilder}* builder = \textcolor{keyword}{new} \hyperlink{classHArDCore2D_1_1Mesh2DBuilder}{Mesh2DBuilder}();
    \hyperlink{classHArDCore2D_1_1Mesh2D}{Mesh2D}* mesh\_ptr = builder->\hyperlink{classHArDCore2D_1_1Mesh2DBuilder_a66379a977bf320cc456c4693b78707c8}{build\_the\_mesh}(vertices, cells);

    std::cout << \textcolor{stringliteral}{"There are "} << mesh\_ptr->\hyperlink{group__Mesh2D_ga49d8009eeb74fd880ef9bb61c37fa672}{n\_cells}() << \textcolor{stringliteral}{" cells in the mesh.\(\backslash\)n"};
\}
\end{DoxyCode}
...
\begin{DoxyCode}
\textcolor{preprocessor}{#include "hybridcore2d.hpp"}
\end{DoxyCode}
...
\rowcolor{\tableheadbgcolor}\textbf{ Symbol name }&\textbf{ Meaning  }\\\cline{1-2}
\endhead
\hyperlink{group__HybridCore2D_ga6faceb46cc896cd6d82a821828518730}{nlocal\+\_\+cell\+\_\+dofs} &The number of degrees of freedom of a cell polynomial (ie. the dimension of the space of polynomials of degree $\le L$ in two variables) \\\cline{1-2}
\hyperlink{group__HybridCore2D_ga6a45e5ff09d21b18874e1446cd9de9bc}{nlocal\+\_\+face\+\_\+dofs} &The number of degrees of freedom of a face polynomial (ie. the dimension of the space of polynomials of degree $\le K$ in one variable) \\\cline{1-2}
\hyperlink{group__HybridCore2D_gaa7f78bc066429a18ad596d45a79d37bc}{nhighorder\+\_\+dofs} &The number of degrees of freedom of a degree $(K+1)$ cell polynomial (ie. the dimension of the space of polynomials of degree $\le K+1$ in two variables) \\\cline{1-2}
\hyperlink{group__HybridCore2D_ga6f7c5ef5103bed8a3e0ecc18d592b1f9}{ngradient\+\_\+dofs} &The dimension of the gradient space of degree $(K+1)$ cell polynomials \\\cline{1-2}
\hyperlink{group__HybridCore2D_gaf97e3efcc37ac51074c7136af3ca517b}{ntotal\+\_\+cell\+\_\+dofs} &The total number of degrees of freedom over all cell polynomials over the entire mesh (i.\+e. the number of cells times the dimension of the space of polynomials of degree $\le L$ in two variables) \\\cline{1-2}
\hyperlink{group__HybridCore2D_ga49a703b6786215e5acf8bb59ecdbc9ea}{ntotal\+\_\+face\+\_\+dofs} &The total number of degrees of freedom over all face polynomials over the entire mesh (i.\+e. the number of edges times the dimension of the space of polynomials of degree $\le K$ in one variables) \\\cline{1-2}
\hyperlink{group__HybridCore2D_gae6e771ab58f248db8f051d47de982cf5}{ninternal\+\_\+face\+\_\+dofs} &The total number of degrees of freedom over all internal faces over the entire mesh \\\cline{1-2}
\hyperlink{group__HybridCore2D_ga728e5b5aa3675a64e7dbc593ed9761f8}{ntotal\+\_\+dofs} &The total number of cell and face degrees of freedom over the entire mesh \\\cline{1-2}
\end{longtabu}
