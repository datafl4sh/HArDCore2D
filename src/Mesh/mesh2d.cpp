...
Mesh2D::Mesh2D() : _mesh_name("mesh-2d"),
					_next_edge_idx(0),
					_cells(0),
					_edges(0),
					_vertices(0),
...
Cell2D* Mesh2D::cell(size_t iC) const {
    if (iC < n_cells()) {
        return _cells[iC];
    } else {
...
Edge2D* Mesh2D::edge(size_t iE) const {
    if (iE < n_edges()) {
        return _edges[iE];
    } else {
...
Vertex2D* Mesh2D::vertex(size_t iV) const {
    if (iV < n_vertices()) {
        return _vertices[iV];
    } else {
...
Cell2D* Mesh2D::b_cell(size_t iC) const {
    if (iC < n_b_cells()) {
        return _b_cells[iC];
    } else {
...
Edge2D* Mesh2D::b_edge(size_t iE) const {
    if (iE < n_b_edges()) {
        return _b_edges[iE];
    } else {
...
Vertex2D* Mesh2D::b_vertex(size_t iV) const {
    if (iV < n_b_vertices()) {
...
Edge2D* Mesh2D::add_edge(std::vector<size_t> vertex_ids, Cell2D* cell) {
    ...
    Edge2D* edge = new Edge2D(iE, vertex_ids, this, cell);
    _edges.push_back(edge);

		// add each vertex to each other's list, and corresponding edge
		Vertex2D* vertex1 = vertex(vertex_ids[0]);
		Vertex2D* vertex2 = vertex(vertex_ids[1]);
...
size_t Mesh2D::n_b_cells() const {
    return _b_cells.size();
}
size_t Mesh2D::n_b_edges() const {
    return _b_edges.size();
}
size_t Mesh2D::n_b_vertices() const {
